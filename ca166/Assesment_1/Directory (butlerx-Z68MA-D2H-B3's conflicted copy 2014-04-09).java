...
class Population //
 {
  Person[] phonebook = new Person[1000]; //an array to store people that supports up to 1000 people
  ...
  void sort(
    ...
    for (int i = 0; phonebook[i] != null; i++) {
      ...
      for (int j = i + 1; j < count; j++) {
        //String tempOne = phonebook[j].surname;
        //String tempTwo = phonebook[minIndex].surname;
        if ((phonebook[j].surname).compareTo(phonebook[minIndex].surname) < 0) minIndex = j;
        else if (phonebook[j].surname.compareTo(tempTwo) < 1) {
          //tempOne = phonebook[j].forename;
          //tempTwo = phonebook[minIndex].forename;
          if ((phonebook[j].forename).compareTo(phonebook[minIndex].forename) < 1) minIndex = j;
        }
      }
