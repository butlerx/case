...
.text
start:  ld r1,N(r0)       ; r1 = N (r1 is like v)

loop:   andi r3,r1,1      ; is the current term (v, r1) odd or even?
        beqz r3,even      ; if statement: branch if term is even (r1 % 2 == 0) (v % 2 == 0)
        nop               ; branch delay slot (use me for useful work)

                          ; if case
odd:    daddi r2,r0,3     ; r2 = 3
        ...
        nop               ; branch delay slot (use me for useful work)
                          ...
                          ; else case
even:   dsrl r1,r1,1      ; divide by 2 (r1 = r1 >> 1) (v = v/2)
                          ; drop through to instruction following the if statement

                          ; after if-then-else
after:  ld r4,N(r0)       ; r4 = maximum value encounterd so far
...
skip:   slti r3,r1,2      ; test for finished (r1 < 2) (v < 2)
